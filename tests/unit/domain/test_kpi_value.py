...
def test_kpi_value_creation() -> None:
    """Verify that KpiValue can be created with a Decimal value."""
    kpi = KpiValue(value=Decimal("1500.50"), unit="USD")
    assert kpi.value == Decimal("1500.50")
    assert kpi.unit == "USD"


def test_kpi_value_is_immutable() -> None:
    """Verify that KpiValue object is immutable."""
    kpi = KpiValue(value=Decimal("100.0"), unit="%")
    with pytest.raises(AttributeError):
        kpi.value = Decimal("200.0")  # type: ignore


def test_kpi_value_formatting_large_numbers() -> None:
    """Verify that KpiValue scales large numbers for display."""
    kpi = KpiValue(value=Decimal("1200000.0"), unit="USD")
...
def test_kpi_value_formatting_percentage() -> None:
    ...
    kpi = KpiValue(value=Decimal("0.856"), unit="%")
...
def test_kpi_value_negative_validation() -> None:
    ...
    kpi = KpiValue(value=Decimal("-10.5"), unit="pts")
    assert kpi.value == Decimal("-10.5")
...
def test_kpi_value_invalid_unit_raises_error() -> None:
    ...
    with pytest.raises(ValueError, match="Unit cannot be empty"):
        KpiValue(value=Decimal("100.0"), unit="")
